...
impl<T> SmoothWeight<T> {
    pub const fn new() -> Self {
        SmoothWeight { items: Vec::new() }
    }

    pub fn add(&mut self, item: T, weight: isize) {
    ...
    fn next_smooth_weighted(&mut self) -> Option<&mut SmoothWeightItem<T>> {
        let mut total = 0;
        ...
        Some(&mut self.items[best_index])
    }
}
...
impl<T: Clone> Iterator for SmoothWeight<T> {
    ...
    fn next(&mut self) -> Option<T> {
        if self.items.len() <= 1 {
            return self.items.first_mut().map(|item| item.item.clone());
        }
